...
def _is_savings_cover_page(full_text):
    """储蓄险封面（盛利 II 等）"""
    return (
        "保障摘要"        in full_text and
        "保障項目"        in full_text and
        ("投保時每年總保費" in full_text or "每年總保費" in full_text) and
        "說明摘要"    not in full_text and
        "愛唯守"      not in full_text  # 排除重疾
    )

def _is_promo_page(full_text):
    """附加推广页（17页后的附页）"""
    return (
        "保證優惠利率推廣計劃" in full_text or
        "建議營業人"           in full_text or
        "Source Code: WEP"     in full_text or
        "保費儲備金帳戶"       in full_text or
        "保費儲備金賬戶"       in full_text
    )
...
def _annotate_cancer(fitz_page, words, policy, font_path):
    ...
    _write_centered(fitz_page, "如果患癌症了，理賠完重疾後",
                    last_y + 18,  RED, function_path, fontsize=12)
...
def redact_personal_info(doc: fitz.Document) -> fitz.Document:
    ...
    for page_num in range(len(doc)):
        ...
        # ═══════════════════════════════════════════
        # 分支 B：附加推广页（独立处理，走完后 continue）
        # ═══════════════════════════════════════════
        if _is_promo_page(text):
            # ① 右上角条形码：照搬主逻辑的右侧遮盖风格
            #    附页通常没有大标题在顶部，直接遮顶部右侧
            redact_rects.append(fitz.Rect(pw * 0.35, 0, pw, ph * 0.10))

            # ② 左下角页脚："建議營業人: XXX  版本  日期  N/2 頁"
            hits_agent = page.search_for("建議營業人")
            if hits_agent:
                r = hits_agent[0]
                redact_rects.append(fitz.Rect(0, r.y0 - 1, pw, ph))
            else:
                redact_rects.append(fitz.Rect(0, ph - 40, pw, ph))

            # ③ 正文中"申請人姓名"行（声明页会有）
            hits_applicant = page.search_for("申請人姓名")
            for r in hits_applicant:
                redact_rects.append(fitz.Rect(r.x0, r.y0 - 1, pw, r.y1 + 1))

            # ④ 正文中"被保人姓名"行（某些附页也有）
            hits_insured = page.search_for("被保人姓名")
            for r in hits_insured:
                redact_rects.append(fitz.Rect(r.x0, r.y0 - 1, pw, r.y1 + 1))
        ...
        is_savings_cover = _is_savings_cover_page(text)
        is_cover         = is_ci_cover or is_savings_cover
        ...
        for rect in redact_rects:
            page.add_redact_annot(rect, fill=WHITE)
        # ★ 用 PIXELS 而不是 NONE：保证覆盖到条形码图像的像素也被擦
        page.apply_redactions(images=fitz.PDF_REDACT_IMAGE_PIXELS)
